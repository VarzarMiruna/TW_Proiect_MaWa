document.getElementById('seara-form').addEventListener('submit', async function(event) {
    event.preventDefault();
  
    const formData = new FormData(event.target);
    const data = new URLSearchParams(formData);
  
    const response = await fetch('/api/seara-sfaturi', {
        method: 'POST',
        body: data
    });
    ...
    sfaturiDiv.innerHTML = `
        <h3>Sfaturi pentru un makeup de seara</h3>
        <ul>
